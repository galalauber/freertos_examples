...
{
   /**
   * Mensagem a ser transmitida via LoRa contendo 253 bytes;
   */
   uint8_t protocol[100];
   ...
   {
      /**
       * Transmite o mesmo comando CMD_READ_COUNT para todos os devices (Slaves), conforme
       * o ranger de endereços em LORA_TOTAL_NODES; 
       */
      for( int i = 1; i <= LORA_TOTAL_NODES; ++i )
      {
         ...
         */
          ...
          protocol[2] = CMD_READ_COUNT;       //comando de leitura da variável Count do receptor;
